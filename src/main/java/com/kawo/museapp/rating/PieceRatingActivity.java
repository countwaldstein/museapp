...
//        Toast.makeText(PieceRatingActivity.this, "Pieces left:"+s,
//                Toast.LENGTH_LONG).show();
    ...
    private void hooksandstuff(final String pathtype, final String pathsubtype) {
        ...
        if (pathtype.equals("PIECE_TYPE")){rbar2.setVisibility(View.GONE);rtext2.setVisibility(View.GONE);}
        ...
        buttrat1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (pathtype.equals("PIECE_TYPE")==true){r2 =-2000;}
...
//                            + Float.toString(mRating.rating3), Toast.LENGTH_SHORT).show();
    ...
    private boolean saveRating(String type, String subtype, int pieceid, float rating1, float rating2, float rating3) {
    ...
    private boolean checkIfRatingExists(String type, String subtype, int pieceid) {
        ...
        if (cursor.moveToFirst()) {
            ...
            Toast.makeText(PieceRatingActivity.this, "Piece not rated", Toast.LENGTH_SHORT).show();
            cursor.close();
            return false;
        }
    ...
    private int getNearestPiece(float prop1, float prop2, float prop3, String type, String subtype) {
        int nearestid = -1;
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                Log.d("getNearestPiece, pkey", cursor.getString(cursor.getColumnIndex(DataBaseHelper.Strings.PIECE_KEY)).substring(0, Math.min(cursor.getString(cursor.getColumnIndex(DataBaseHelper.Strings.PIECE_KEY)).length(), 1)));
            } while (cursor.moveToNext());
        }
        cursor.close();
        ...
        while (i < arrpiece.size()) {
            ...
            if (type.equals("PIECE_TYPE")){array[1] = 0;}
            else
